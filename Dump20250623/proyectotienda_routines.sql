...
SELECT SUM(CASE 
	...
	,SUM(CASE 
        ...
        CASE 
            WHEN mov.Tipo_pago = 1 AND mov.Tipo_movimiento != 2 THEN 1
            ELSE NULL
        END
    ) AS CANTIDAD_CREDITO,
    COUNT(
        CASE 
            WHEN mov.Tipo_pago = 2 AND mov.Tipo_movimiento != 2 THEN 1
            ELSE NULL
        END
...
AND NOT EXISTS (
      SELECT 1
      FROM movimientos AS anulacion
      WHERE anulacion.Tipo_movimiento = 2
        AND anulacion.Idpago = mov.Idpago
  );
END ;;
DELIMITER ;
...
-- Dump completed on 2025-06-29 15:01:26
