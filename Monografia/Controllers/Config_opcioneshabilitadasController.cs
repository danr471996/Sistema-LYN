...
{
    public class Config_opcioneshabilitadasController : Controller
    {
        ...
        // GET: Config_opcioneshabilitadas
        ...
        public ActionResult Index(Modelo_Config Modelo)
        {
            foreach (var item in Modelo.Lista_opciones)
            {
                ...
                Actualizar_datos(item.ID_OP, item.SELECCIONADO_OP, item.DETALLE_EXT1);
            }
            Modelo_actual.Lista_opciones = Obtener_opciones();
            return View("index", Modelo_actual);
        }      

        private string Actualizar_datos(string ID,bool SELECCIONADO,string EXT1)
        {
