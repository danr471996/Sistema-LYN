    ...
    public class ConfigController : Controller
    {
        ...
        private string introducir_perfil(string perfil, string codigo, DateTime fechaalta, string usuarioalta)
        {
            ...
            try
            {
                ...
                string[] splitPermisos = codigo.Split('|').Where(x => !string.IsNullOrEmpty(x)).ToArray();            
                foreach (string id_permiso in splitPermisos)
                {
                    query = "INSERT INTO permisos_perfil (Id_permiso,Id_perfil) values (@Id_permiso,@Id_perfil)";
                    using (MySqlCommand comando = new MySqlCommand(query, mysqlcon))
                    {
                        comando.Parameters.AddWithValue("@Id_perfil", id_usuario_perfil);
                        comando.Parameters.AddWithValue("@Id_permiso", id_permiso);
                        comando.ExecuteNonQuery();
                    }
                }
                mysqlcon.Close();
        ...
        private string Actualizar_perfil(int idperfil, string perfil, string codigo)
        {
            ...
            try
            {
                ...
                string query = "UPDATE usuarios_perfiles SET Descripcion_perfil = @DescripcionPerfil WHERE Id_perfil=@idperfil";
                using (MySqlCommand comando = new MySqlCommand(query, mysqlcon))
                {
                    comando.Parameters.AddWithValue("@DescripcionPerfil", perfil);
                ...
                query = "DELETE FROM permisos_perfil where Id_perfil = @idperfil";
                using (MySqlCommand comando = new MySqlCommand(query, mysqlcon))
                {
                    comando.Parameters.AddWithValue("@idperfil", idperfil);
                    comando.ExecuteNonQuery();
        ...
        private List<checkboxs> obtener_checkboxs()
        {
            MySqlConnection mysqlcon = new MySqlConnection(mysqlconeccion);
            mysqlcon.Open();
            MySqlCommand comando = new MySqlCommand("SELECT * FROM lista_permisos order by SUBSTR(id_permiso FROM 1 FOR 1),CAST(SUBSTR(id_permiso FROM 2) AS UNSIGNED),inicio_grupo", mysqlcon);
            MySqlDataReader lector = comando.ExecuteReader();
        ...
        private List<checkboxs> obtener_checkboxs_mod(int idperfil)
        {
            ...
            MySqlConnection mysqlcon = new MySqlConnection(mysqlconeccion);
            mysqlcon.Open();
            MySqlCommand comando = new MySqlCommand("SELECT lp.Id_permiso,lp.Descripcion,!ISNULL(pp.Id_permiso) AS SELECCIONADO,lp.GRUPO,lp.INICIO_GRUPO FROM lista_permisos as lp left join permisos_perfil as pp on pp.Id_perfil = '" + idperfil + "' and pp.Id_permiso = lp.Id_permiso order by SUBSTR(lp.id_permiso FROM 1 FOR 1), CAST(SUBSTR(lp.id_permiso FROM 2) AS UNSIGNED), lp.inicio_grupo", mysqlcon);
            MySqlDataReader lector = comando.ExecuteReader();
